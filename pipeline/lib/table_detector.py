    ...
    if candidates_bottom:
        ...
        if y_search_bottom_end > y_search_bottom_start:
            ...
            for row_offset in range(search_strip_bottom.shape[0] - 1, -1, -1):
                ...
                dark_pixels = np.sum(row < 128)
                if dark_pixels > width * 0.3:
                    # Add 30px margin below the detected border for safety
                    blok3_y_end = min(y_search_bottom_start + row_offset + 30, height)
                    break
        else:
            # No border detected, use keyword position + 30px margin
            blok3_y_end = min(best_bottom['y'] + 30, height)
    else:
        # No keyword detected, add 30px margin to default bottom
        blok3_y_end = height
...
def detect_table_region(image: np.ndarray) -> Optional[Tuple[int, int, int, int]]:
    ...
    try:
        ...
        if best_bottom:
            ...
            print(f"    → Lower boundary: y={blok3_y_end} (+30px margin for safety)")
        else:
